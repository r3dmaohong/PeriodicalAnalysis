...
require_once 'reader.php';
  ...
  echo "<title>計算國家出現次數</title>";
  
  echo "<style>h { color: red; font-style: italic; font-weight: 900;}</style>";
  
  echo "<br>【計算國家出現次數】<br>";
  echo "<br>";
  echo "<form action='0508_country_count.php' method='get'>";
  ...
  echo "</form>";
	...
	if(in_array($if_exist_str,$dep_name)){
		$count_contry_one[$value[0]]++;
			...
			$if_exist_str2 = $value[0].$value[1];
			array_push($dep_name,$if_exist_str2);
			array_push($dep_name2,$value[1]);
			
			echo "　　➔　　　已搜尋到國家『".$value[1]."』<br>";
	...
	echo "<br>　　➔　　　共有".count($b_array_dep_array)."個國家";
